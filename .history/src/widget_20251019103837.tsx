...
async function chat(prompt: string, context: string, lang: LanguageCode): Promise<string> {
  ...
  if (!API_KEY) {
    ...
    const responses: Record<LanguageCode, Record<string, string>> = {
      en: {
        ...
        'what is your name': 'I am SarvamBot, your AI assistant powered by Sarvam AI.',
        'who are you': 'I am SarvamBot, a multilingual AI assistant that can help you in English, Hindi, and Tamil.',
        'default': `I'm a demo bot. To get real AI responses, please add your Sarvam API key to the .env file.`
      },
      hi: {
        'default': 'मैं एक डेमो बॉट हूं। वास्तविक AI प्रतिक्रियाओं के लिए, कृपया .env फ़ाइल में अपनी Sarvam API कुंजी जोड़ें।'
      },
      ta: {
        'default': 'நான் ஒரு டெமோ போட். உண்மையான AI பதில்களுக்கு, தயவுசெய்து .env கோப்பில் உங்கள் Sarvam API விசையைச் சேர்க்கவும்.'
      }
    };
    
    const lowerPrompt = prompt.toLowerCase().trim();
    const langResponses = responses[lang];
    return langResponses[lowerPrompt] || langResponses['default'] || `Echo: ${prompt}`;
  }
  ...
  try {
    ...
    return 'API unavailable. Please check your API key and try again.';
  }
}
