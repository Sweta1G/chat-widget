...
async function chat(prompt: string, context: string, lang: LanguageCode): Promise<string> {
  ...
  try {
    const response = await fetch('https://api.sarvam.ai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `${context} Always respond in the user's language. Language code: ${lang}`
          },
          {
            role: 'user',
            content: prompt
    ...
    return `I apologize, but I'm having trouble connecting to the AI service. Please try again. (Error: ${error instanceof Error ? error.message : 'Unknown'})`;
  }
}
